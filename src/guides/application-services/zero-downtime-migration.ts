...
export const ZERO_DOWNTIME_MIGRATION_STEPS: Omit<MigrationStep, 'status'>[] = [
    ...
    },
    {
        ...
        description:
            'Automate your Cloudflare configuration using Infrastructure as Code (IaC) with Terraform. Ready-to-use Terraform templates (Provider v5) are available below for download. Use cf-terraforming to export existing configuration and integrate with CI/CD pipelines for version control and automated deployments. After 7-14 days of stable operation, decommission legacy DNS infrastructure.',
        estimatedTime: '1-2 hours',
        checkpoints: [
            { id: 'terraform-reviewed', label: 'Cloudflare Terraform provider v5 documentation reviewed', completed: false, optional: true },
        ...
        terraformFiles: [
            ...
            { name: 'variables.tf', description: 'Input variables with defaults and validation', path: '/terraform/application-services/variables.tf' },
            { name: 'outputs.tf', description: 'Output values for zone info and resources', path: '/terraform/application-services/outputs.tf' },
            { name: 'dns.tf', description: 'DNS records (A, AAAA, CNAME, MX, TXT)', path: '/terraform/application-services/dns.tf' },
            { name: 'ssl.tf', description: 'SSL/TLS certificates and settings', path: '/terraform/application-services/ssl.tf' },
            { name: 'waf.tf', description: 'WAF managed rules, OWASP, custom rules', path: '/terraform/application-services/waf.tf' },
            { name: 'rules.tf', description: 'Redirects, rewrites, header modifications', path: '/terraform/application-services/rules.tf' },
            { name: 'cache.tf', description: 'Cache rules and tiered cache', path: '/terraform/application-services/cache.tf' },
            { name: 'security.tf', description: 'Managed transforms and security headers', path: '/terraform/application-services/security.tf' },
            { name: 'terraform.tfvars.example', description: 'Example variables file with documentation', path: '/terraform/application-services/terraform.tfvars.example' },
        ],
        phase: 3,
        phaseTitle: 'Enable Proxy & Automation',
    },
